...
func solution(_ str1:String, _ str2:String) -> Int {
    ...
    for _ in 0..<string1.count-1 {
        let elem = string1.removeFirst().description + string1.first!.description
        if elem.isOnlyEnglish() {
            array1.append(elem.lowercased())
        }
    }
    
    var array2: [String] = []
    for _ in 0..<string2.count-1 {
        let elem = string2.removeFirst().description + string2.first!.description
        if elem.isOnlyEnglish() {
            array2.append(elem.lowercased())
        }
    }
    ...
    let overlapped: [String] = set1.filter { set2.contains($0) }
    let over1: [Int] = overlapped.map { _ in array1.filter { array2.contains($0) }.count }
    let over2: [Int] = overlapped.map { _ in return array2.filter { array1.contains($0) }.count }
    
    var minCnt: Int {
        var cnt = 0
        for i in 0..<overlapped.count {
            cnt += min(over1[i], over2[i])
        }
        return cnt
    }
    var maxCnt: Int {
